...
{#if !!description}
    <Text text={description} style={{ textAlign: 'center' }} wrapperTag='div' />
{/if}

{#if !!pros && pros.length > 0}
    <h3>ОБО МНЕ:</h3>
    <UnorderedList list={pros} theme="green" />
{/if}
...
{#if !!process}
    ...
    <Text text={process} style={{ textAlign: 'center', fontSize: 14 }} wrapperTag='div' />
{/if}
